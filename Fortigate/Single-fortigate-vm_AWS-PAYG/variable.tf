...
variable "bucket" {
    description = "use s3 bucket for bootstrap. Either true or false"
    type    = bool
}

variable "fgtvm-vpc_endpoint-to-s3" {
    description = "s3 bucket name"
    type = string
}

variable "bootstrap-fgtvm" {
  description = "FGTVM config in plaintext Format for S3 Bucket"
  type    = string
}
...
variable "license" {
  description = "license file for the active fgt. Change to your own byol license file, license.lic"
  type    = string
}

variable "license_format" {
    description = "BYOL License format to create FortiGate-VM. Provide the license type for FortiGate-VM Instances, either token or file."
    type = string
}
...
variable "arch" {
    description = "instance architect. Either arm or x86"
}

variable "license_type" {
  description = "License Type to create FortiGate-VM. Provide the license type for FortiGate-VM Instances, either byol or payg."
  type = string
}

variable "size" {
    description = "instance type needs to match the architect. For detail, refer to https://aws.amazon.com/ec2/instance-types/"
    type = string
}

variable "s3-bucket-name" {
    description = "s3 bucket for fgtvm"
    type = string
}

variable "key_name" {
  description = "keypair file name"
  type = string
}
...
variable "fgtvm-public-rt" {
    description = "fortigate vm public route-table name"
    type = string
}

variable "fgtvm-private-rt" {
  description = "fortigate vm private route-table name"
  type = string
}

variable "fgtiamprofile" {
  description = "FortiGate instance IAM Profile name"
  type = string
}

variable "iam-role-fgtvm" {
  description = "fortigate-vm iam role name"
  type = string
}

variable "iam-policy-fgtvm" {
    description = "fortigate-vm iam policy name"
    type = string
}

variable "public-sg-ingress-cidr" {
    description = "ingress cidr for public security group"
    type = string
}

variable "public-sg-egress-cidr" {
  description = "egress cidr for public security group"
  type = string
}
...
variable "private-sg-ingress-cidr" {
  description = "ingress cidr for private security group"
  type = string
}

variable "private-sg-egress-cidr" {
    description = "egress cidr for private security group"
    type = string
}
...
variable "fgvm_sm" {
    description = "secret manager name to store fgtvm admin credentials"
    type = string
}